...
class InfluxDb2ExtensionIT {

    private static final @NotNull String INFLUXDB_TOKEN = "integration-test-token";
    ...
    private final @NotNull InfluxDBContainer<?> influxDB =
            new InfluxDBContainer<>(OciImages.getImageName("influxdb:v2")).withAdminToken(INFLUXDB_TOKEN)
                    .withOrganization(INFLUXDB_ORGANIZATION)
    ...
    void testMetricsAreForwardedToInfluxDB() {
        final var clientOptions = InfluxDBClientOptions.builder()
                .url(influxDB.getUrl())
                .authenticateToken(INFLUXDB_TOKEN.toCharArray())
                .org(INFLUXDB_ORGANIZATION)
                .bucket(INFLUXDB_BUCKET)
                .build();
        try (final var influxDBClient = InfluxDBClientFactory.create(clientOptions)) {
    ...
    private long getMetricMax(final @NotNull InfluxDBClient client, final @NotNull String metric) {
        final var flux = """
                from(bucket: "%s")
                    ...
                    |> max()""".formatted(INFLUXDB_BUCKET, metric);
        final var tables = client.getQueryApi().query(flux, INFLUXDB_ORGANIZATION);
        long max = 0;
        for (final var table : tables) {
            for (final var record : table.getRecords()) {
                final var value = getValue(record.getValue());
                if (value > max) {
                    max = value;
                }
            }
